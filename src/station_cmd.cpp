 ...
 * @param flags Command flags.
 * @return A succeeded command if the road can be cleared, a failed command with the relevant error otherwise.
 */
static CommandCost CanRemoveRoadWithStop(TileIndex tile, DoCommandFlag flags)
{
	/* Water flooding can always remove road stops. */
...
CommandCost ClearTile_Station(TileIndex tile, DoCommandFlag flags)
{
	...
	switch (GetStationType(tile)) {
		...
		case STATION_BUS:
			if (IsDriveThroughStopTile(tile)) {
				/* Check if we can remove the underlying road. */
